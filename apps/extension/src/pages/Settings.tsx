...
export default function Settings({ layout }: SettingsProps) {
  ...
  return (
    <FormProvider {...methods}>
      <form id="Setting" name="Setting" action="" className={cn(isOptions && "grid grid-cols-[minmax(0,1fr)_240px] gap-6 max-[900px]:block")} onSubmit={doSubmit}>
        ...
        <aside className={cn(isOptions ? "rounded-[14px] border border-slate-200 bg-white p-4 shadow-sm max-[900px]:mt-4" : "mt-4")}>
          {isOptions && (
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs leading-4 text-slate-500">保存会同步到扩展存储，重置只恢复表单默认值。</p>
            </div>
          )}
          <button className="w-full rounded-xl bg-[#101828] px-4 py-3 text-sm font-semibold text-white shadow-[0_14px_28px_rgba(16,24,40,0.22)] transition-all hover:-translate-y-0.5 hover:bg-[#1d2939] active:translate-y-0 active:bg-[#344054] disabled:opacity-60" type="submit">
            保存设置
          </button>
          <button
            className="mt-3 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600 transition-all hover:border-slate-300 hover:bg-white hover:text-slate-900 active:bg-slate-100 disabled:opacity-60"
            type="button"
            onClick={() => {
